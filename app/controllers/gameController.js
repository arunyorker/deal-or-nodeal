...
// gameService.getAmount();
gameService.getAmount().then(function(response)
{
	console.log(response.data[0].left);
	//response.data.left.length
	var left=[];
	var right=[];
		for(var i=0;i<response.data[0].left.length;i++)
		{
		 	var leftObject ={
		 					"money":response.data[0].left[i],
		 					"selected":false
		 					}
		 ...
		 for(var i=0;i<response.data[0].right.length;i++)
		 {
		 	var rightObject ={
		 					"money":response.data[0].right[i],
		 					"selected":false
		 					}
...
gameService.getAmount().then(function(response){
		for(var i=0;i<response.data[0].left.length;i++)
		{
			boxAmounts.push(response.data[0].left[i]);
		}
		for(var i=0;i<response.data[0].right.length;i++)
		{
			boxAmounts.push(response.data[0].right[i]);
		}
...
$scope.eliminateBox1=function(a){
	if(a.openCaseClass=="" && animated==false)
	{
		...
		a.number='';
		$timeout(function(){
			a.openINR='assets/images/inr3.png';
